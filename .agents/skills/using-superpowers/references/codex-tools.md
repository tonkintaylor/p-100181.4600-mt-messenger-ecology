...
Agent skills reference named agent types like `code-reviewer`. Codex does not have a named agent registry — `spawn_agent` creates generic agents from built-in roles (`default`, `explorer`, `worker`).
...
| ------------------------------------------------ | ------------------------------------------------------------------------------- |
| `Task tool (`code-reviewer`-type)`               | `spawn_agent(agent_type="worker", message=...)` with `code-reviewer.md` content |
...
See `using-creating-worktrees` Step 0 and `finishing-a-development-branch` Step 1 for how each skill uses these signals.
...
File-reading tools may wrap content in display code fences (` ```language ` and ` ``` `) that are **not** part of the actual file. Never include these fence delimiters in `oldString` when editing files — they are presentation-only formatting added by the tool. If unsure, verify the actual file content via the shell (e.g., `cat <file>`) before composing `oldString`.
