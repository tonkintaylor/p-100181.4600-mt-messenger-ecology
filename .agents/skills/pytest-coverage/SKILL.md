...
description: >-
  Run pytest with coverage analysis and increase coverage toward 100%. Use when
  the user asks to check test coverage, find untested code, add missing tests,
  or improve coverage — even if they don't say 'pytest' explicitly. Also use
  when the user says 'what's not tested', 'coverage report', or 'add tests for
  uncovered lines'.
compatibility:
  platform: universal
metadata:
