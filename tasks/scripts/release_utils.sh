...
check_format_release() {
    ...
    then
        echo "Error: Release tag $release does not match the format $RELEASE_FMT_MSG" >&2
        exit 1
    fi
}
...
check_nodupl_releases() {
    ...
    if ! [ -z "$dupl_tags" ]
    then
        echo "Error: Duplicate release tags detected: run \`git tag --list\`" >&2
        exit 1
    fi
}
...
le_release() {
    ...
    chunks1=($(chunk_release "$release1")) || return 1
    chunks2=($(chunk_release "$release2")) || return 1
...
check_are_ordered_releases() {
    ...
    then
        echo "Error: Release tag $release1 is out of order with respect to $release2" >&2
        exit 1
    fi
}
...
check_are_sequential_releases() {
    ...
    then
        ...
        then
            ...
            echo "Error: Release tag $release1 is not sequential with respect to $release2!" >&2
            exit 1
        fi
...
get_next_custom_release() {
    ...
    then
        ...
        echo "Error: cannot get user input to query custom tag!" >&2
        exit 1
    fi
...
get_next_sequential_release() {
    ...
    then
        ...
        local chunks
        chunks=($(chunk_release "$latest_release")) || exit 1
